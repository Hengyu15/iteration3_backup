/**
 * HTML5游戏源码教程
 * http://longwenjunjie.github.io/games
 *
 * QQ群:236030520
 *
 */

var g_GameZOder = {bg: 0, ui: 1, front: 100};//游戏中显示的层级
var g_GameStatus={normal:0,stop:1,gameOver:2};//游戏的状态，0：正常，1：暂停 2:游戏结束
//var flag_b1 = 0;
//var flag_b2 = 0;
//var flag_b3 = 0;
//var flag_b4 = 0;

var GameScene = cc.Scene.extend({
    gameLayer: null, //游戏层 ，所有的元素都添加在这个层
    mushroom: null,  //蘑菇
    bear: null, //熊
    lblScore: null,//分数标签
    score: 0,//分数
    lblLives: null, //生命标签，以绘图形式表现生命数
    lives: 5, //生命数
    btnStart: null,  //开始按钮
    leafList: [],//椰子数组
    acornList: [],//橡子数组
    flowerList: [],//花数组
    waterdropList: [],
    waterdropList2: [],
    waterdropList3: [],
    flag_b1: [0],
    flag_b2: [0],
    flag_b3: [0],
    flag_b4: [0],
    winSize: cc.size(480, 320),
    gameStatus: 0,//游戏的状态，0：正常，1：暂停
    onEnter: function () {
        this._super();  //调用父类的同名方法，这里是调用cc.Scene的onEnter
        //一般在这里自己写进入场景后的操作
        this.initData();
        //cc.view.setDesignResolutionSize(1920,1080,cc.ResolutionPolicy.EXACT_FIT);
        this.schedule(this.update, 0);
    },
    /**
     * 初始化数据
     */
    initData: function () {
        //this.winSize = cc.Director.getInstance().getWinSize();//获取当前的绘图窗口的大小
        this.winSize = cc.winSize;
        this.lives = 5;//初始化生命值
        this.score = 0;//初始化分数
        
        this.gameStatus =  g_GameStatus.stop; //默认暂停
//      this.flag_b1 = 0;
//      this.flag_b2 = 0;
//      this.flag_b3 = 0;
//      this.flag_b4 = 0;

        //原则上来说，不能直接在Scene中添加Sprite，  一般做法是Scene添加Layer ，然后在Layer中添加Layer或者Sprite
        //添加Layer
        this.gameLayer = cc.Layer.create();
        this.addChild(this.gameLayer);

        //添加背景
        var bg = cc.Sprite.create(s_dryland);
        bg.setAnchorPoint(cc.p(0, 0));
        this.gameLayer.addChild(bg, g_GameZOder.bg);

        //添加蘑菇
        this.mushroom = new MushroomSprite();
        this.mushroom.setAnchorPoint(cc.p(0.5, 0));
        this.mushroom.setPosition(cc.p(240, 0));
        this.gameLayer.addChild(this.mushroom, g_GameZOder.ui);

        //添加熊
        this.bear = new BearSprite();
        this.gameLayer.addChild(this.bear, g_GameZOder.ui);
        this.bear.initData();
        this.bear.curSence = this;

        //添加分数背景
        var bgScore = cc.Sprite.create(s_score);
        bgScore.setAnchorPoint(cc.p(1, 1));
        bgScore.setPosition(cc.p(this.winSize.width, this.winSize.height));
        this.gameLayer.addChild(bgScore, g_GameZOder.ui);
        //添加分数
        this.lblScore = cc.LabelTTF.create("0", "Arial", 18);
        this.lblScore.setPosition(cc.p(this.winSize.width - 30, this.winSize.height - 21));
        this.lblScore.setColor(cc.color(0, 0, 255));
        this.gameLayer.addChild(this.lblScore, g_GameZOder.ui);

        //生命
        this.lblLives = cc.Sprite.create(s_lives5);
        this.lblLives.setAnchorPoint(cc.p(0, 1));
        this.lblLives.setPosition(cc.p(0, this.winSize.height));
        this.lblLives.setScale(0.35);
        this.gameLayer.addChild(this.lblLives, g_GameZOder.ui);

        //开始按钮
        var start1 = cc.Sprite.create(s_start_button);
        var start2 = cc.Sprite.create(s_start_button);
        //cc.MenuItemSprite 参数1：正常状态时显示的Sprite 参数2：摁下选中状态时显示的Sprite 参数3：执行函数 参数4：一般传入this
        this.btnStart = cc.MenuItemSprite.create(start1, start2, this.startGame, this);
        this.btnStart.setScale(0.5);

        var infoMenu = cc.Menu.create(this.btnStart);
        this.gameLayer.addChild(infoMenu, g_GameZOder.front);

        //初始化奖品
        this.initAcorn();
        this.initFlower();
        this.initLeaf();
        this.initWaterdrop();
        

    },
    startGame: function () {
        //假如处于结束状态则先重置数据
        if(this.gameStatus == g_GameStatus.gameOver){
               this.resetData();
        }
        this.gameStatus = g_GameStatus.normal; //设置游戏状态为正常
        this.bear.beginRotate(); //开始旋转
        this.btnStart.setVisible(false);//隐藏开始按钮
    },
    overGame:function(){
        this.gameStatus = g_GameStatus.gameOver; //设置游戏状态为结束
        this.bear.stopRotate(); //开始旋转
        this.btnStart.setVisible(true);//显示开始按钮
    },
    resetData:function(){
        //重设生命值
        this.lives = 5;
        this.flag_b1 = [0];
        this.flag_b2 = [0];
        this.flag_b3 = [0];
        this.flag_b4 = [0];
        this.lblLives = cc.Sprite.create(s_lives5);
        this.lblLives.setAnchorPoint(cc.p(0, 1));
        this.lblLives.setPosition(cc.p(0, this.winSize.height));
        this.lblLives.setScale(0.35);
        this.gameLayer.addChild(this.lblLives, g_GameZOder.ui);

        //重置蘑菇
        this.mushroom.setPosition(cc.p(240, 0));
        //重置熊
        this.bear.initData();

        //重置叶子
        for (var i = 0; i < this.leafList.length; i++) {
            var prize = this.leafList[i];
            prize.isHit= false;
            prize.setVisible(true);
        }
        //重置花
        for (var i = 0; i < this.flowerList.length; i++) {
            var prize = this.flowerList[i];
            prize.isHit= false;
            prize.setVisible(true);
            
        }
        //重置橡子
        for (var i = 0; i < this.acornList.length; i++) {
            var prize = this.acornList[i];
            prize.isHit= false;
            prize.setVisible(true);
        }
        for (var i = 0; i < this.waterdropList.length; i++) {
            var prize = this.waterdropList[i];
            prize.setPosition(cc.p(30 + i * 30, 270));
            prize.setVisible(false);
            prize.setOpacity(255);
           }
        for (var i = 0; i < this.waterdropList2.length; i++) {
            var prize = this.waterdropList2[i];
            prize.setPosition(cc.p(60 + i * 30, 245));
            prize.setVisible(false);
            prize.setOpacity(255);
           }
        for (var i = 0; i < this.waterdropList3.length; i++) {
            var prize = this.waterdropList3[i];
            prize.setPosition(cc.p(90 + i * 30, 220));
            prize.setVisible(false);
            prize.setOpacity(255);
           }
        var bg = cc.Sprite.create(s_dryland);
        bg.setAnchorPoint(cc.p(0, 0));
        this.gameLayer.addChild(bg, g_GameZOder.bg);
        this.score = 0;
        this.lblScore.setString(this.score.toString());
        
    },
    //减少生命值
    reduceLives: function () {
        this.lives -= 1; //减少1生命值
        this.lblLives.initWithFile("res/lives" + this.lives + ".png"); //根据生命数改变显示相关的图片
        this.lblLives.setScale(0.35);
        this.lblLives.setAnchorPoint(cc.p(0, 1));//重置锚点

        if (this.lives <= 0) {
            //生命值少0，游戏结束
            this.overGame();
        } else {
            this.gameStatus = g_GameStatus.normal;
            this.bear.initData();  //重设熊的位置和状态
        }
    },
    //初始化橡子
    initAcorn: function () {
        var left = 0; //左边距离
        var space = 30; //间距
        for (var i = 1; i <= 15; i++) {
            //添加15个
            var prize = new AcornPrize();
            prize.initData();
            prize.setPosition(cc.p(left + i * space, 270));
            this.gameLayer.addChild(prize, g_GameZOder.ui);
            this.acornList.push(prize);  //放入数组，用来取出检测碰撞
        }
    },
    //初始化花
    initFlower: function () {
        var left = 30;  //左边距离
        var space = 30;  //间距
        for (var i = 1; i <= 13; i++) {
            //添加13个
            var prize = new FlowerPrize();
            prize.initData();
            prize.setPosition(cc.p(left + i * space, 245));
            this.gameLayer.addChild(prize, g_GameZOder.ui);
            this.flowerList.push(prize);  //放入数组，用来取出检测碰撞
//          var moveby = cc.MoveBy(1,cc.p(0,10));
//          this.flowerList[i].File.runAction(moveby);
        }
    },
    //初始化叶子
    initLeaf: function () {
        var left = 60;   //左边距离
        var space = 30;   //间距
        for (var i = 1; i <= 11; i++) {
            //添加11个
            var prize = new LeafPrize();
            prize.initData();
            prize.setPosition(cc.p(left + i * space, 220));
            this.gameLayer.addChild(prize, g_GameZOder.ui);
            this.leafList.push(prize);  //放入数组，用来取出检测碰撞
        }
    },
    initWaterdrop: function(){
    	var left = 0;
    	var space = 30;
    	for (var i = 1; i <= 15; i++) {
            //添加15个
            var prize = new WaterdropPrize();
            prize.initData();
            prize.setPosition(cc.p(left + i * space, 270));
            this.gameLayer.addChild(prize, g_GameZOder.ui);
            prize.setVisible(false);
            this.waterdropList.push(prize);
        }
    	for (var i = 1; i <= 13; i++) {
            //添加15个
            var prize = new WaterdropPrize();
            prize.initData();
            prize.setPosition(cc.p(30 + i * space, 245));
            this.gameLayer.addChild(prize, g_GameZOder.ui);
            prize.setVisible(false);
            this.waterdropList2.push(prize);
        }
    	for (var i = 1; i <= 11; i++) {
            //添加15个
            var left = 60;
    		var space = 30;
            var prize = new WaterdropPrize();
            prize.initData();
            prize.setPosition(cc.p(60 + i * space, 220));
            this.gameLayer.addChild(prize, g_GameZOder.ui);
            prize.setVisible(false);
            this.waterdropList3.push(prize);
        }
    },
    

    
//  move: function (t){
//  	t -=20;
//  	return t;
//  }
    drop: function (i) {
        //for(var j = 0;  j< 5; j++){
            setTimeout(this.waterdropList[i].y-=20,1000);
            //setInterval(this.waterdropList[i].y-=20,1000*(j+1));
            //setTimeout(move(this.waterdropList[i].y),1000);
            //}
    },
    addScore: function (point) {
        this.score += point;
        this.lblScore.setString(this.score.toString());
    },
    update: function (dt) {
        if (this.gameStatus != g_GameStatus.normal) {
            return;
        }
        this.bear.update(dt);
        //判断熊与蘑菇碰撞
        this.bear.collide(this.mushroom);

        //判断熊与叶子碰撞
        for (var i = 0; i < this.leafList.length; i++) {
            var prize = this.leafList[i];
            //判断没被碰撞则检测
            if (!prize.isHit) {
                if (this.bear.collide(prize)) {
                    prize.setVisible(false);  //隐藏
                    this.waterdropList3[i].setVisible(true);
                    var actionMoveBy = cc.moveBy(2,cc.p(0,-133));
                    var actionFadeOut = cc.fadeOut(0.5);
                    var sequence = cc.sequence(actionMoveBy,actionFadeOut);
                    this.waterdropList3[i].runAction(sequence);
                    prize.isHit = true;  //设置已碰撞，下次循环不检测
                    this.addScore(prize.point); //添加分数
                }
            }
        }
        //判断熊与叶子碰撞
        for (var i = 0; i < this.flowerList.length; i++) {
            var prize = this.flowerList[i];
            //判断没被碰撞则检测
            if (!prize.isHit) {
                if (this.bear.collide(prize)) {
                    prize.setVisible(false);  //隐藏
                    this.waterdropList2[i].setVisible(true);
                    var actionMoveBy = cc.moveBy(2,cc.p(0,-133));
                    var actionFadeOut = cc.fadeOut(0.5);
                    var sequence = cc.sequence(actionMoveBy,actionFadeOut);
                    this.waterdropList2[i].runAction(sequence);
                    prize.isHit = true;   //设置已碰撞，下次循环不检测
//                  var bigImg = document.createElement("img");
//                  bigImg.src = "res/owl1.png";
                    
                    this.addScore(prize.point);  //添加分数
                }
            }
        }
        //判断熊与叶子碰撞
        for (var i = 0; i < this.acornList.length; i++) {
            var prize = this.acornList[i];
            //判断没被碰撞则检测
            if (!prize.isHit) {
                if (this.bear.collide(prize)) {
                    prize.setVisible(false);   //隐藏
                    this.waterdropList[i].setVisible(true);
                    var actionMoveBy = cc.moveBy(2,cc.p(0,-133));
                    var actionFadeOut = cc.fadeOut(0.5);
                    var sequence = cc.sequence(actionMoveBy,actionFadeOut);
                    this.waterdropList[i].runAction(sequence);
                    //this.waterdropList[i].setPosition(cc.pAdd(this.getPosition(), cc.pAdd((0,20), dt)));
                    //this.flowerList[i].File.runAction(moveby);
                    //this.waterdropList[i].timer(); 
                    //this.waterdropList[i].y=200;
                    //this.waterdropList[i].move();
                  //for(var x=0; x<5;x++){
                  //	this.drop(i);
                  //}
//					setInterval(function(){this.waterdropList[i].y+=20},3000)
                    
                    
                    //setTimeout(this.waterdropList[i].y+=20,1000)
                    prize.isHit = true;      //设置已碰撞，下次循环不检测
                    this.addScore(prize.point);   //添加分数
                    
                }
            }
        }
        if(this.flag_b1[0] == 0 && (this.score==150 || this.score==160 ||this.score == 170)){
                    	
                    	this.flag_b1[0] += 1;
                    	//this.score = 320
                    	//this.gameLayer.removeChild(bg,g_GameZOder.bg);
                    	var bg = cc.Sprite.create(s_dryland2);
        				bg.setAnchorPoint(cc.p(0, 0));
        				this.gameLayer.addChild(bg, g_GameZOder.bg);
        				
                   }
        
//      if(flag_b1 == 1){
//      	//this.flag_b2 = 1;
//                  	//this.gameLayer.removeChild(bg,g_GameZOder.bg);
//                  	var bg = cc.Sprite.create(s_dryland3);
//      				bg.setAnchorPoint(cc.p(0, 0));
//      				this.gameLayer.addChild(bg, g_GameZOder.bg);
//      }
        
        if(this.flag_b2[0] == 0 && (this.score==310 || this.score==320 ||this.score == 330)){
                    	
                    	this.flag_b2[0] = 1;
                    	//this.gameLayer.removeChild(bg,g_GameZOder.bg);
                    	var bg = cc.Sprite.create(s_dryland3);
        				bg.setAnchorPoint(cc.p(0, 0));
        				this.gameLayer.addChild(bg, g_GameZOder.bg);
                    }
        if(this.flag_b3[0] == 0 && (this.score==500 || this.score==510 ||this.score == 520)){
                    	
                    	this.flag_b3[0] = 1;
                    	//this.gameLayer.removeChild(bg,g_GameZOder.bg);
                    	var bg = cc.Sprite.create(s_grassland);
        				bg.setAnchorPoint(cc.p(0, 0));
        				this.gameLayer.addChild(bg, g_GameZOder.bg);
        				
                    }
        if(this.flag_b4[0] == 0 && this.score==820){
                    	
                    	this.flag_b4[0] = 1;
                    	var bg = cc.Sprite.create(s_forest1);
        				bg.setAnchorPoint(cc.p(0, 0));
        				this.gameLayer.addChild(bg, g_GameZOder.bg);
        				this.overGame();
                    }
    }
});